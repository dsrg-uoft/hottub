      ...
      while (fgets(line, 1023, output)) {
        ...
        if (line[0] != '[') {
          ...
          if (class_name != NULL) {
            ...
            //TODO: run safe classes before client connects, run unsafe class re-initialization when client connects before main
            // another option is to not run static analysis and just re-init everything before main
            /* len - 2: \n + \0 */
